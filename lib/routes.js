...
module.exports = {
    ...
    configure: function (app) {
			app.post('/api/authUser',function (req, res,next) {
				security(req,res);CreditSociety.authUser(req.body,req.decoded, res);
			});
		...
		/* EMPLOYEE DETAILS */
			...
			app.get('/api/ListEmployees/', function (req, res) {
				CreditSociety.ListEmployees(req, res);
			});
			
			app.get('/api/getEmployeesDetails/:empid', function (req, res) {
				CreditSociety.getEmployeesDetails(req.params.empid, res);
			});
			
			app.post('/api/VerifyrequiredFields/', function (req, res) {
				CreditSociety.VerifyrequiredFields(req.body, res);
			});
			
			app.post('/api/SaveEmployeesDetails', upload.single('file'),function (req, res) {
            CreditSociety.SaveEmployeesDetails(req.file,req.body, res);
			});
