...
var opts = {
  ...
  function encrypt(text) {
	...
	exports.Listusers= function(req, res) {
		...
		()=>{
			employees.find({},{'username':1,'password':1}).exec(function (err, result) {
				if(err)
				{
	...
	exports.authUser= function(userdetails, res) {
		...
		()=>{
			try{
			employees.find({'username':userdetails.username,'password':userdetails.password},{'username':1,'profilepic':1,'role':1}).exec(function (err, result) {
				if(err)
				{
					console.log(err);
					res.send({success:false,message:'Somthing went wrong, Please try again.'});
				...
				else
				{
					...
					if(result.length == 1)
					{
								...
								var token = jwt.sign(payload, app.get('superSecret'), {
									...
									//res.cookie('branchid', String(result[0].branch[0]), { expires: new Date(expires), httpOnly: true });
	...
	/* BRANCH DETAILS */
			...
			if(req.decoded.success == true)
		{
			mongoose.connect(process.env.MONGOLAB_URI,opts).then(
			()=>{
				branches.find({}).populate('createdby','username').exec(function (err, result) {
					if(err)
					{
						res.send({status:1,message:'Somthing went wrong, Please try again!'});
						mongoose.disconnect();
	...
	/* AREA DETAILS */
		...
		mongoose.connect(process.env.MONGOLAB_URI,opts).then(
		()=>{
			   areas.find({}).populate('branchid','branchname').exec(function (err, result) { 
						console.log(err);
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
	...
	exports.getAreaDetails= function(req, res) {
		if(req.decoded.success == true)
		{
		mongoose.connect(process.env.MONGOLAB_URI,opts).then(
		()=>{
			var areaid = req.params.areaid;
			areas.find({_id:areaid}).populate('branchid','branchname').exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
	...
	/* GROUP DETAILS */
		...
		mongoose.connect(process.env.MONGOLAB_URI,opts).then(
		()=>{
			 groups.find({}).populate('areaid','areaname').populate('branchid','branchname').exec(function (err, result) { 
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
	...
	/* EVENTS DETAILS */
			...
			if(req.decoded.success == true)
		{
			mongoose.connect(process.env.MONGOLAB_URI,opts).then(
			()=>{
				 events.find({}).populate('branch','branchname').populate('group','groupname').populate('areaid','areaname').exec(function (err, result) { 
					if(err)
					{
						res.send({status:1,message:'Somthing went wrong, Please try again!'});
						mongoose.disconnect();
	...
	/* ACCOUNT PLANS */
		...
		mongoose.connect(process.env.MONGOLAB_URI,opts).then(
		()=>{
			   acplans.find({}).populate('actypeid','accounttype').exec(function (err, result) { 
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
	...
	exports.ListEmployees= function(req, res) {
		...
		()=>{
			...
			if(req.Loggedinuser.role == 'Admin' || req.Loggedinuser.role == 'User')
				var filterqry ={'branch':req.Loggedinuser.branchid}
			employees.find(filterqry,{'firstname':1,'lastname':1,'midname':1,'gander':1,'doj':1,'role':1,'branch':1,'contactdetails':1}).populate('branch','branchname').exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
	...
	exports.SaveEmployeesDetails= function(req, res) {
		...
		()=>{
			...
			if(employeepic)
			{
				if(!empdet.employeedetails.documentsdetails)
				{
					empdet.employeedetails.documentsdetails = {};
				}
	...
	exports.ListMembers= function(req, res) {
		...
		()=>{
			...
			if(req.Loggedinuser.role == 'Admin' || req.Loggedinuser.role == 'User')
				var filterqry ={'branch':req.Loggedinuser.branchid}
			members.find(filterqry,{'firstname':1,'lastname':1,'midname':1,'depositeamount':1,'documentsdetails.aadhaar':1,'contactdetails.mobile1':1,'isagent':1}).populate({ path: 'branch', select: 'branchname' }).exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
	...
	exports.SaveMembersACDetails= function(req, res) {
		...
		()=>{
			...
			membersaccountdDetails.branchid = req.body.branch[0];
	...
	exports.ListAgents= function(req, res) {
		...
		()=>{
			...
			agents.find(filterqry,{'createddate':1,'activestatus':1}).populate('memberid').populate('branch','branchname').populate('areaid','areaname').populate('group','groupname').exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
	...
	exports.GetAgentsDetails= function(req, res) {
		if(req.decoded.success == true)
		{
		mongoose.connect(process.env.MONGOLAB_URI,opts).then(
		()=>{
				...
				var filterqry ={'_id':req.params.agentid}
			
			agents.find(filterqry).populate('memberid').exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
		...
		exports.ActiveDeactiveFeatures= function(req, res) {
		if(req.decoded.success == true)
		{
		mongoose.connect(process.env.MONGOLAB_URI,opts).then(
		()=>{
			if(req.body.isagent == 0)
				var actvsts = 1;
			if(req.body.isagent == 1)
				var actvsts = 0;
			members.updateOne({ _id: req.body._id },{'isagent':actvsts},{ multi: true }, function(err) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
				...
				{
					agents.find({memberid:req.body._id}).exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
				...
				else
				{
					...
					agents.updateOne({ _id:result[0]._id },{'activestatus':actvsts},{ multi: true }, function(err) {
						if(err)
						{
							res.send({status:1,message:'Somthing went wrong, Please try again!'});
							mongoose.disconnect();
						...
						{
							res.send({status:0,message:'Status Changed'});
							mongoose.disconnect();
						}
					});
					}
	...
	exports.SaveMemberAsAgent= function(req, res) {
		if(req.decoded.success == true)
		{
		mongoose.connect(process.env.MONGOLAB_URI,opts).then(
		()=>{
				...
				{
					var memberid = req.body.memberid._id
				}
				else
				{
			...
			agents.find({memberid:memberid}).exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
				...
				else
				{
					if(result != null && result.length == 1)
					{
							...
							agents.updateOne({ _id: result[0]._id },{group:req.body.group[0],areaid:req.body.areaid[0],branch:req.body.branch[0]},{ multi: true }, function(err) {
								if(err)
								{
									res.send({status:1,message:'Somthing went wrong, Please try again!'});
									mongoose.disconnect();
					...
					else
					{
						...
						var agentdetails = new agents({group:req.body.group[0],areaid:req.body.areaid[0],branch:req.body.branch[0],memberid:req.body._id,activestatus:1});
						
						agentdetails.save(function(err,result) {
					if(err)
					{
						res.send({status:1,message:'Somthing went wrong, Please try again!'});
						mongoose.disconnect();
					...
					{
						members.updateOne({ _id: req.body._id },{'isagent':1},{ multi: true }, function(err) {
	...
	exports.ListAreasOnBranch= function(req, res) {
		...
		()=>{
			...
			areas.find({'branchid':branchid},{'areaname':1}).exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
	...
	exports.ListGroupsOnArea= function(req, res) {
		...
		()=>{
			var areaid = req.params.areaid;
			groups.find({'areaid':areaid},{'groupname':1}).exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
