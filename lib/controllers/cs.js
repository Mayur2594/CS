...
var opts = {
	...
	exports.SaveEmployeesDetails= function(employeepic,employeeDetails, res) {
		mongoose.connect(process.env.MONGOLAB_URI,opts).then(
		()=>{
			
			if(employeepic)
			{
				employeeDetails.employeedetails.profilepic = employeepic.filename;
			}
			 var empDetails = new employees(employeeDetails.employeedetails);
			if(employeeDetails._id)
			{
				employees.update({ _id: employeeDetails._id }, employeeDetails,{ multi: true }, function(err) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
