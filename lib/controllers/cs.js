...
var opts = {
  ...
  function encrypt(text) {
			...
			employees.find({'username':userdetails.username,'password':userdetails.password},{'username':1,'profilepic':1,'role':1}).exec(function (err, result) {
				...
				else
				{
					if(result.length > 1 || result.length <= 0)
					{
							var payload = {
								...
								var token = jwt.sign(payload, app.get('superSecret'), {
									expiresIn: 1 // expires in 24 hours = 86400
								});
								var vertoken = null;
								var tokennew = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VycGFzcyI6IjMyMSIsImlhdCI6MTUzODA0NjIwMSwiZXhwIjoxNTM4MDQ2MjAyfQ.ySpNgQhF-FvCbAHuB5UhwCAicRHF61ASmlZrAt4W72k";
								jwt.verify(tokennew, app.get('superSecret'), function(err, decoded) {			
									if (err) {
										return res.json({ success: false, message: 'Failed to authenticate token.' });		
									} else {
										// if everything is good, save to request for use in other routes
									vertoken	= decoded;	
									res.json({success:false,message:'Login cridentials does not matched.',token,vertoken});
									}
								});
					...
					if(result.length == 1)
					{
								...
								res.json({success:true,data:result[0],message:'logged in successfully.'});
					}
	...
	exports.VerifyrequiredFields= function(data, res) {
		mongoose.connect(process.env.MONGOLAB_URI,opts).then(
		()=>{
