  ...
  function encrypt(text) {
	...
	exports.ListMembers= function(req, res) {
		...
		()=>{
			...
			if(req.Loggedinuser.role == 'Admin' || req.Loggedinuser.role == 'User')
				var filterqry ={'branch':req.Loggedinuser.branchid}
			members.find(filterqry,{'firstname':1,'lastname':1,'midname':1,'depositeamount':1,'documentsdetails.aadhaar':1,'contactdetails.mobile1':1}).populate({ path: 'branch', select: 'branchname' }).exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
