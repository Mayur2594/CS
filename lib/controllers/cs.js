...
var opts = {
  ...
  function encrypt(text) {
	...
	exports.ListGuarantors= function(req, res) {
		...
		mongoose.connect(process.env.MONGOLAB_URI,opts).then(
		()=>{
			var selectedmemberid = req.params.memberid;
			var depositedamt = req.params.depositedamt;
			
			if(req.Loggedinuser.role == 'Superadmin')
				var filterqry ={}
			if(req.Loggedinuser.role == 'Admin' || req.Loggedinuser.role == 'User')
			{
				var filterqry = {'branch.id':req.Loggedinuser.branchid,'_id': { $ne: selectedmemberid },'depositeamount': { $eq: depositedamt }};
			}
	...
	exports.getMembersAccounts= function(req, res) {
		...
		()=>{
			...
			membersacc.find({'memberid._id':memberid}).exec(function (err, result) {
				if(err)
				{
					res.send({status:1,message:'Somthing went wrong, Please try again!'});
					mongoose.disconnect();
